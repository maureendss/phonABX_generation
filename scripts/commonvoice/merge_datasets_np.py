...
#    val.to_csv(os.path.join(args.output_dir, "val.tsv"), sep="\t")
    ...
    lang_item = pd.read_csv(args.path_item_1, sep=' ')
                       
    #create gender items
    gender_item = convert_phone_item(lang_item, val, target="gender")


    

    gender_item.to_csv(os.path.join(args.output_dir, "natpref_gender.item"), sep=" ", index=False)

    # python scripts/commonvoice/merge_datasets.py /scratch2/mde/projects/phonABX_generation/dataset/en/en.item /scratch1/data/raw_data/commonvoice/cv-corpus-7.0-2021-07-21/en/validated.tsv /scratch2/mde/projects/phonABX_generation/dataset/fr/fr.item /scratch1/data/raw_data/commonvoice/cv-corpus-7.0-2021-07-21/fr/validated.tsv test 
